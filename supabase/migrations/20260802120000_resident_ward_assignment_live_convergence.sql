-- Converges the previously recorded Resident-Ward migration with the live UUID schema.
-- profiles.ward_assignment stays exclusively for Student assignments.
...
set note = concat_ws(
  ...
  case when nullif(trim(today_plan), '') is not null then 'Plan: ' || trim(today_plan) end
)
...
create index if not exists resident_ward_assignments_ward_idx
  on public.resident_ward_assignments (ward_id, resident_id);

drop trigger if exists set_resident_ward_assignments_updated_at on public.resident_ward_assignments;
create trigger set_resident_ward_assignments_updated_at
before update on public.resident_ward_assignments
...
grant select on table public.resident_ward_assignments to authenticated;

drop policy if exists "resident_ward_assignments_admin_all" on public.resident_ward_assignments;
create policy "resident_ward_assignments_admin_all"
...
create policy "resident_ward_assignments_self_select"
on public.resident_ward_assignments
...
for select to authenticated
using (
  (public.current_profile_role() = 'resident' and public.resident_has_ward_access(ward_id::text))
...
begin
  ...
  from unnest(normalized_resident_ids) candidate(id)
  left join public.profiles profile
    on profile.id = candidate.id
   and profile.role = 'resident'
   and coalesce(profile.is_active, true)
  where profile.id is null;

  if invalid_count > 0 then
    raise exception 'One or more selected residents are unavailable';
  end if;

  delete from public.resident_ward_assignments
  where ward_id::text = target_ward_id;
...
revoke all on function public.resident_has_ward_access(text) from public;
grant execute on function public.resident_has_ward_access(text) to authenticated;
