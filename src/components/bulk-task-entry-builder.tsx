...
export function BulkTaskEntryBuilder({
  ...
  const [selectedWardId, setSelectedWardId] = useState(defaultWardId);
  ...
  const visibleSummaries = useMemo(
    () => wardSummaries.filter((summary) => (selectedWardId ? summary.ward.id === selectedWardId : true)),
    [selectedWardId, wardSummaries],
  );
  ...
  const defaultOwnerProfiles = selectedWardId ? (profilesByWard[selectedWardId] ?? []) : [];
    ...
    <div className="space-y-6">
          ...
          <Field label="Ward">
            <SelectBox
              ...
              onChange={(event) => {
                ...
                const nextProfiles = profilesByWard[nextWardId] ?? [];
                if (!nextProfiles.some((profile) => profile.id === defaultOwnerId)) {
                  setDefaultOwnerId("");
                }
              }}
            >
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <Field label="Default owner">
              ...
              >
                ...
                {defaultOwnerProfiles.map((profile) => (
      ...
      <form action={submitAction} className="space-y-6">
        ...
        {visibleSummaries.map((summary) => (
          <section key={summary.ward.id} className="space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-muted">Ward</p>
                <h3 className="mt-1 text-lg font-semibold text-foreground">{summary.ward.name}</h3>
              </div>
              <div className="text-sm text-muted">{summary.patients.length} active patients</div>
            </div>

            <div className="space-y-4">
              {summary.patients.map((patient) => {
                const patientRows = rowsByPatient[patient.id] ?? [];
                const wardProfiles = profilesByWard[summary.ward.id] ?? [];
        ...
        <div className="flex justify-end">
          <SubmitButton pendingLabel="Creating tasks...">Create tasks in bulk</SubmitButton>
        </div>
      </form>
    </div>
  );
